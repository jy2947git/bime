...
<ul>
    <li class="buttonBar right">
        ...
        <c:if test="${param.from == 'list' and param.method != 'Add'}">
            <input type="submit" class="button" name="delete" onclick="bCancel=true;return confirmDelete('item')"
    ...
    <li class="info">
        <c:choose>
            <c:when test="${param.from == 'list'}">
                <p><fmt:message key="item.update.message"/></p>
            </c:when>
            <c:otherwise>
                <p><fmt:message key="item.add.message"/></p>
            </c:otherwise>
        </c:choose>
    </li>
...
<script type="text/javascript">
    Form.focusFirstElement($('managedItemForm'));
